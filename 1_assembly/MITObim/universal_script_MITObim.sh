...
# Path to mira (MITObim requires mira in PATH)
...
# --- MITObim command construction ---
# Original command: MITObim.pl -start 1 -end 30 -sample $papka -ref $papka -readpool $readseq --quick $ref &> log
MITOBIM_CMD="$MITOBIM_BIN -start 1 -end 30 -sample $papka_name -ref $papka_name -readpool $readseq --quick $ref &> log"
...
if [ $MONITOR_EXIT -eq 0 ]; then
    ...
    # MITObim log may be in the file 'log' (which we redirected to)
    if [ -f "log" ]; then
